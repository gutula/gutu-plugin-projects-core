...
describe("projects-core lifecycle integration", () => {
  ...
  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "projects-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      correlationId: "projects-core:corr",
      processId: "project-delivery-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "projects-core:demo",
      exceptionId: "projects-core:exception",
