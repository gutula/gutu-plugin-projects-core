export type ProjectsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildProjectsCoreSqliteMigrationSql(options: ProjectsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "projects_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getProjectsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getProjectsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildProjectsCoreSqliteRollbackSql(options: ProjectsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "projects_core_");
  return [
...
export function getProjectsCoreSqliteLookupIndexName(tablePrefix = "projects_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getProjectsCoreSqliteStatusIndexName(tablePrefix = "projects_core_"): string {
