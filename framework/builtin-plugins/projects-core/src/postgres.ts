export type ProjectsCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildProjectsCoreMigrationSql(options: ProjectsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "projects_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getProjectsCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getProjectsCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildProjectsCoreRollbackSql(options: ProjectsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "projects_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "projects_core";
  return [
...
export function getProjectsCoreLookupIndexName(): string {
  return "projects_core_primary_lookup_idx";
}

export function getProjectsCoreStatusIndexName(): string {
  return "projects_core_exception_status_idx";
}
